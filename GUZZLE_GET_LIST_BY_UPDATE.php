<?php
	//import Guzzle Framework - autoload.php
	require "..\\..\\..\\php\\vendor\\autoload.php";

	//url, appKeys, requestId, and type for calling REST API
	$url="https://api-sms.cloud.toast.com/sms/v2.1";
	$appKeys="/appKeys/EGAHwbWtW692uzLs";
	$type="/message-results";
	
	//set time for getting results
	$startUpdateDate="2018-12-20";
	$endUpdateDate="2018-12-20";
	$startTime="00:00:00";
	$endTime="10:40:00";
	
	//encode time data
	$encoded_startUpdateDate=urlencode($startUpdateDate.' '.$startTime);
	$encoded_endUpdateDate=urlencode($endUpdateDate.' '.$endTime);
	
	//setting a header
	$headers=array(
		'Content-Type: application/json;charset=UTF-8'
	);
	
	//set REST API Client
	$client=new GuzzleHttp\Client();
	
	//call REST API
	$response=$client->request('GET', $url.$appKeys.$type."?startUpdateDate=".$encoded_startUpdateDate."&endUpdateDate=".$encoded_endUpdateDate, [
		'header'=>$headers
	]);
	
	//print result
	print_r(json_encode(json_decode($response->getBody(), true)));
?>